...
def main() -> None:
    ...
    if not (os.path.isfile("train_data.csv") and os.path.isfile("val_data.csv")):
        print("Error: 'train_data.csv' and 'val_data.csv' not found.")
        print("Please run 'train_validation_split.py' first to generate these files.")
        sys.exit(1)

    # Load pre-saved train/validation CSVs
    train_df = load_data("train_data.csv")
    val_df   = load_data("val_data.csv")
    ...
    np.save("trained_model.npy", model_data)
    print("Model saved to 'trained_model.npy'")
